...
app.post('/process-images', upload.array('images', 10), async (req, res) => {
  try {
    ...
    for (const file of files) {
      ...
      // Parse and store extracted Hindi text
      text.split('\n').forEach((line) => {
        const name = line.trim(); // Extract each line as a name
        if (name) {
          rows.push({ Name: name, Aadhaar: '', Mobile: '' }); // Aadhaar and Mobile are placeholders
        }
      });
